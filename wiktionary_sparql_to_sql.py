...
from queries import WIKTIONARY_ADJECTIVES_SMALL, WIKTIONARY_ADVERBS_SMALL, WIKTIONARY_DB_ADJECTIVES_SMALL_CREATE, WIKTIONARY_DB_ADJECTIVES_SMALL_INSERT, WIKTIONARY_DB_ADVERBS_SMALL_CREATE, WIKTIONARY_DB_ADVERBS_SMALL_INSERT, WIKTIONARY_DB_NOUNS_SMALL_CREATE, WIKTIONARY_DB_NOUNS_SMALL_INSERT, WIKTIONARY_DB_PROPER_NOUNS_SMALL_CREATE, WIKTIONARY_DB_PROPER_NOUNS_SMALL_INSERT, WIKTIONARY_DB_VERBS_SMALL_CREATE, WIKTIONARY_DB_VERBS_SMALL_INSERT, WIKTIONARY_NOUNS, WIKTIONARY_DB_NOUNS_CREATE, WIKTIONARY_DB_NOUNS_INSERT, WIKTIONARY_NOUNS_CNT, WIKTIONARY_NOUNS_SMALL, WIKTIONARY_PROPER_NOUNS_SMALL, WIKTIONARY_VERBS_SMALL
...
def get_wiktionary_nouns(limit: int, offset: int, db_name:str):    
    sparql = SPARQLWrapper("https://kaiko.getalp.org/sparql")
    query = WIKTIONARY_NOUNS
    query = query.replace('{LIMIT_VAR}', str(limit))
    query = query.replace('{OFFSET_VAR}', str(offset))
    sparql.setQuery(query)
    sparql.setReturnFormat(JSON)
    ...
    data_to_insert = [(result["s"]["value"],
                        ...
                        result["writtenRep"]["value"], 
                        result["gender"]["value"], 
                        result["otherFormNomSingular"]["value"], 
                        result["nomWrittenRepSingular"]["value"], 
                        result["otherFormNomPlural"]["value"], 
                        result["nomWrittenRepPlural"]["value"], 
                        result["otherFormAkkSingular"]["value"], 
                        result["akkWrittenRepSingular"]["value"], 
                        result["otherFormAkkPlural"]["value"], 
                        result["akkWrittenRepPlural"]["value"], 
                        result["otherFormDatSingular"]["value"], 
                        result["datWrittenRepSingular"]["value"], 
                        result["otherFormDatPlural"]["value"], 
                        result["datWrittenRepPlural"]["value"], 
                        result["otherFormGenSingular"]["value"], 
                        result["genWrittenRepSingular"]["value"], 
                        result["otherFormGenPlural"]["value"], 
                        result["genWrittenRepPlural"]["value"])
                        for result in results["results"]["bindings"]]
    
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.executemany(WIKTIONARY_DB_NOUNS_INSERT, data_to_insert)
    conn.commit()
    conn.close()
